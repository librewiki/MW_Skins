...
if (!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 4.4.007_PHP4 (http://www.tcpdf.org)');
		...
		*/
		function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false) {
			...
			if (!isset($this->images[$file])) {
				...
				if ($type == '') {
					if (!preg_match('/\?action=/', $file))
						$fileinfo = pathinfo($file);
					else
						$fileinfo = array('extension'=>'png');
				...
				case 'img': {
					if (isset($tag['attribute']['src'])) {
						...
						if ($tag['attribute']['src'][0] == '/') {
							if (!preg_match('/\?action=/', $tag['attribute']['src']))
